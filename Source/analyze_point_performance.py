...
from Source.Performance.Point.landing import landing
    ...
    for index, row in point_performance_input.iterrows():
        ...
        if onlyobjectives == False:
            if point_performance_input.loc[index,"Name"] == "INSTANTANEOUS_TURN":
                point_performance_data.loc["INSTANTANEOUS_TURN (deg/s)","Value"] = instantaneous_turn(parameters[0], parameters[1], parameters[2], parameters[3], reference_area, (ac_weight - fuel_usable*(100.0-parameters[4])/100.0), engine_input, aerodynamic_data, design_input)
            elif point_performance_input.loc[index,"Name"] == "MAXIMUM_MACH_NUMBER":
                point_performance_data.loc["MAXIMUM_MACH_NUMBER","Value"] = maximum_mach_number(parameters[0], parameters[1], reference_area, (ac_weight - fuel_usable*(100.0-parameters[2])/100.0), engine_input, aerodynamic_data, design_input)
            elif point_performance_input.loc[index,"Name"] == "SEP":
                point_performance_data.loc["SEP","Value"] = sep(parameters[0], parameters[1], parameters[2], reference_area, (ac_weight - fuel_usable*(100.0-parameters[3])/100.0), engine_input, aerodynamic_data, design_input)
            elif point_performance_input.loc[index,"Name"] == "ACCELERATE":
                point_performance_data.loc["ACCELERATE","Value"] = accelerate(parameters[0], parameters[1], parameters[2], parameters[3], reference_area, (ac_weight - fuel_usable*(100.0-parameters[4])/100.0), engine_input, aerodynamic_data, design_input)
            ...
            elif point_performance_input.loc[index,"Name"] == "LANDING":
                point_performance_data.loc["LANDING","Value"] = landing(parameters[0], parameters[1], parameters[2], parameters[3], reference_area, (ac_weight - fuel_usable*(100.0-parameters[4])/100.0), engine_input, aerodynamic_data, design_input)
        #Objectives Calculated    
        if point_performance_input.loc[index,"Name"] == "SUSTAINED_TURN":
            point_performance_data.loc["SUSTAINED_TURN (g)","Value"] = sustained_turn(parameters[0], parameters[1], parameters[2], reference_area, (ac_weight - fuel_usable*(100.0-parameters[3])/100.0), engine_input, aerodynamic_data, design_input)
        elif point_performance_input.loc[index,"Name"] == "SUPERCRUISE_MACH_NUMBER":
            point_performance_data.loc["SUPERCRUISE_MACH_NUMBER","Value"] = supercruise_mach_number(parameters[0], parameters[1], reference_area, (ac_weight - fuel_usable*(100.0-parameters[2])/100.0), engine_input, aerodynamic_data, design_input)
